...
async def analyze_with_ai(user_message: str, market_data: Optional[Dict] = None):
    ...
    try:
        ...
        system_message = """Du bist CHAiNALYZE, ein hochentwickelter Trading-Analyse-Assistent mit Zugriff auf:
...
async def load_historical_data(
    ...
    """Load historical OHLCV data for analysis"""
    try:
        user = await get_current_user(authorization)
        ...
        historical_data = await ai_data_module.load_historical_data(symbol, timeframe, limit)
        
        return {
            'status': 'success',
            'symbol': symbol,
            'timeframe': timeframe,
            'bars_loaded': len(historical_data),
            'data': [asdict(candle) for candle in historical_data],
            'timestamp': datetime.now(timezone.utc).isoformat()
        }
...
async def calculate_technical_indicators(
    ...
    try:
        ...
        symbol = request.get('symbol', 'BTC/USDT')
        timeframe = request.get('timeframe', '1h')
        indicators = request.get('indicators', ['rsi', 'sma', 'ema'])
        
        # Calculate indicators
        indicator_data = await ai_data_module.calculate_technical_indicators(symbol, timeframe, indicators)
...
@api_router.get("/ai-data/status")
async def get_ai_data_status(authorization: str = Header(...)):
    """Get AI Data Module status"""
    try:
        user = await get_current_user(authorization)
        ...
        status_info = await ai_data_module.get_system_status()
        
        return {
            'status': 'success',
            'ai_data_module': status_info,
            'timestamp': datetime.now(timezone.utc).isoformat()
        }
        
    except Exception as e:
        logger.error(f"AI Data Module status error: {e}")
        return {
            'status': 'error',
            'message': 'AI Data Module not fully initialized',
            'timestamp': datetime.now(timezone.utc).isoformat()
        }

# === AI EVOLUTION STATUS ENDPOINTS ===

@api_router.get("/ai/evolution/status")
async def get_ai_evolution_status(authorization: str = Header(...)):
    """Get detailed AI Evolution system status"""
    try:
        user = await get_current_user(authorization)
        ...
        evolution_status = await self_evolving_ai.get_current_status()
        ...
        logger.error(f"AI Evolution status error: {e}")
        return {
            'status': 'error',
            'message': str(e),
            'initialized': False,
...
@api_router.get("/ai/coding/plugins")
async def get_coding_plugins_status(authorization: str = Header(...)):
    """Get Self-Coding AI plugins status"""
    try:
        user = await get_current_user(authorization)
        ...
        # Get plugin status
        plugins = await self_coding_ai.list_active_plugins()
        ...
        logger.error(f"Coding plugins status error: {e}")
        return {
            'status': 'error',
            'message': str(e),
            'plugins': [],
...
async def startup_event():
    ...
    logger.info("Starting CHAiNALYZE...")
...
async def shutdown_event():
    ...
    client.close()
    logger.info("CHAiNALYZE shutdown complete")