...
async def focused_test():
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post('https://ai-trade-hub-4.preview.emergentagent.com/api/auth/login', json=login_data) as response:
            if response.status == 200:
                ...
                # Test 1: AI Analysis
                async with session.post('https://ai-trade-hub-4.preview.emergentagent.com/api/ai-trading/analyze?symbol=BTC/USDT&context=comprehensive_test', headers=headers) as ai_response:
                ...
                async with session.post(f'https://ai-trade-hub-4.preview.emergentagent.com/api/ai-trading/chat-command?command={command}', headers=headers) as chat_response:
                ...
                async with session.post('https://ai-trade-hub-4.preview.emergentagent.com/api/trading/order', json=order_data, headers=headers) as order_response:
                ...
                async with session.get('https://ai-trade-hub-4.preview.emergentagent.com/api/realtime/latest') as rt_response:
